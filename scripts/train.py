...
def _get_train_data_loader(batch_size, training_dir, **kwargs):
    logger.info("Get train data and data loader")
    dataset = datasets.ImageFolder(training_dir, transform=transforms.Compose([
...
def _get_test_data_loader(test_batch_size, testing_dir, **kwargs):
    logger.info("Get test data and data loader")
    dataset = datasets.ImageFolder(testing_dir, transform=transforms.Compose([
        transforms.ToTensor()
    ]))
    test_loader = torch.utils.data.DataLoader(dataset, batch_size=test_batch_size, **kwargs)
    return test_loader
...
def train(args):
    ...
    train_loader = _get_train_data_loader(args.batch_size, args.training_dir, **kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--testing_dir', type=str, default=os.environ['SM_CHANNEL_TESTING'])
